...
def add(x: Union[int, float], y: Union[int, float]) -> Union[int, float]:
    try:
        if not isinstance(x, (int, float)):
            raise TypeError(
                f"x는 숫자 타입이어야 합니다. 현재 타입: {type(x).__name__}"
            )
        if not isinstance(y, (int, float)):
            raise TypeError(
                f"y는 숫자 타입이어야 합니다. 현재 타입: {type(y).__name__}"
            )
        return x + y
...
def power(base: Union[int, float], exponent: Optional[int] = None) -> Union[int, float]:
    ...
    try:
        if not isinstance(base, (int, float)):
            raise TypeError(
                f"base는 숫자 타입이어야 합니다. 현재 타입: {type(base).__name__}"
            )
        if exponent is not None and not isinstance(exponent, int):
            raise TypeError(
                f"exponent는 정수 타입이거나 None이어야 합니다. 현재 타입: {type(exponent).__name__}"
            )

        exp = (
            exponent if exponent is not None else 2
        )  # Optional 처리: None이면 기본값 2 사용
        return base**exp
    except TypeError as e:
        print(f"타입 오류: {e}")
        raise
